...
export const fetchWorkspaces = createAsyncThunk('workspace/fetchWorkspace', async ({ getToken }) => {
    try {
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchWorkspaces.fulfilled,(state,action)=>{
             ...
             if(action.payload.length > 0){
                const localStorageCurrentWorkspaceId = localStorage.getItem('localStorageCurrentWorkspaceId');
                if(localStorageCurrentWorkspaceId){
                    const findWorkspace = action.payload.find((w)=> w.id === 
                localStorageCurrentWorkspaceId);
                if(findWorkspace){
